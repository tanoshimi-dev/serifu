...
#### 1-6. テスト (Unit Test + E2E Test)

テスト公開前にアプリの品質を担保するため、ユニットテストとE2Eテストを実施する。

**Unit Test (Flutter):**
- モデルクラスのシリアライズ/デシリアライズ
- リポジトリ層のAPIレスポンスパース
- AuthService のトークン保存・復元ロジック
- AppTheme の定数値検証

**Unit Test (Backend / Go):**
- ハンドラーのHTTPテスト (httptest)
- モデルのバリデーション
- 認証ミドルウェアのトークン検証
- ソーシャルログインのフロー

**E2E Test (Flutter Integration Test):**
- ログイン → ホーム画面遷移
- クイズ一覧表示 → クイズ詳細 → セリフ投稿
- いいね・コメント操作
- プロフィール表示・編集
...
**ツール:**
- Flutter: `flutter_test` (unit), `integration_test` (E2E)
- Go: `testing` + `httptest` (unit), `testify` (アサーション)
...
| 2026-02-17 | 1-3〜1-5 実装完了。テスト工程 (1-6) を追加、テスト公開準備を 1-7 に繰り下げ |
